...
import { Pflanze } from '../pflanze/entity/pflanze.entity.js';
import { entities } from '../pflanze/entity/entities.js';
...
const database = (db?.name as string | undefined) ?? Pflanze.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Pflanze.name.toLowerCase();
