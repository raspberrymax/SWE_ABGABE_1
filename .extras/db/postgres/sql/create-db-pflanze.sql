...
-- (3) In Windows das Verzeichnis "C:/Zimmermann/volumes/postgres/tablespace/pflanze" anlegen
--     Siehe .extras\compose\postgres\compose.yml
-- (4) 2. PowerShell:
--     cd .extras\compose\backend\postgres
--     docker compose exec db bash
--        chown postgres:postgres /var/lib/postgresql/tablespace
--        chown postgres:postgres /var/lib/postgresql/tablespace/pflanze
...
-- (7) 2. PowerShell:
--     docker compose exec db bash
--        psql --dbname=postgres --username=postgres --file=/sql/create-db-pflanze.sql
--        psql --dbname=pflanze --username=pflanze --file=/sql/create-schema-pflanze.sql
--        exit
...
CREATE ROLE pflanze LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE pflanze;

GRANT ALL ON DATABASE pflanze TO pflanze;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE pflanzespace OWNER pflanze LOCATION '/var/lib/postgresql/tablespace/pflanze';
