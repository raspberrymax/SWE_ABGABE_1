...
> ❗ Vor dem 1. Start von PostgreSQL muss man die Skripte `create-db-pflanze.sql`
> und `create-schema-pflanze.sql` aus dem Verzeichnis `.extras\db\postgres\sql`
...
> nach `C:\Zimmermann\volumes\postgres\csv\pflanze`.

> ❗ Vor dem 1. Start von MySQL muss man das Skript `create-db-pflanze.sql` aus
...
> dem Verzeichnis `.extras\db\mysql\csv` nach `C:\Zimmermann\volumes\mysql\csv\pflanze`.
...
`db.populate` in `src\config\resources\pflanze.yml` wird festgelegt, ob die
(Test-) DB `pflanze` neu geladen wird.
...
- Im Eingabefeld _Name_ `pflanze` und im Eingabefeld _Summary_ z.B.
...
und wählt aus dem Verzeichnis `.extras\postman` die Datei `pflanze.postman_environment.json`
aus. Jetzt hat man die Umgebung `pflanze` mit der Variablen `base_url` und dem
Wert `https://localhost:3000` angelegt.

Im Environment `pflanze` muss man die Variable `client_secret` auf den Wert setzen,
der in Keycloak beim _Realm acme_ in _Clients > pflanze-client > Credentials_
...
```powershell
    npm exec jest --detectOpenHandles --errorOnDeprecated `
      --forceExit --runTestsByPath '__tests__\pflanze\pflanze-GET.controller.test.ts'
```
...
```powershell
    ...
    docker build --tag juergenzimmermann/pflanze:2025.4.1-bookworm .

    # Alpine
    Get-Content Dockerfile.alpine | docker run --rm --interactive hadolint/hadolint:2.12.1-beta-debian
    docker build --tag juergenzimmermann/pflanze:2025.4.1-alpine --file Dockerfile.alpine .
```
...
```powershell
    docker history juergenzimmermann/pflanze:2025.4.1-bookworm
    docker history juergenzimmermann/pflanze:2025.4.1-alpine
```
...
```powershell
    docker inspect juergenzimmermann/pflanze:2025.4.1-bookworm
    docker inspect juergenzimmermann/pflanze:2025.4.1-alpine
```
...
```powershell
    docker sbom juergenzimmermann/pflanze:2025.4.1-bookworm
    docker sbom juergenzimmermann/pflanze:2025.4.1-alpine
```
...
```powershell
    ...
    # PowerShell fuer pflanze-Server mit Bookworm-Image zzgl. DB-Server und Mailserver
    docker compose up

    # Nur zur Fehlersuche: weitere PowerShell für bash
    cd .extras\compose\pflanze
    docker compose exec pflanze bash
        id
        env
        exit
    ...
    # 2. Powershell: pflanze-Server einschl. DB-Server und Mailserver herunterfahren
    cd .extras\compose\pflanze
    docker compose down
```
...
```powershell
    docker scout quickview juergenzimmermann/pflanze:2025.4.1-bookworm
    docker scout quickview juergenzimmermann/pflanze:2025.4.1-alpine
```
...
```powershell
    docker scout cves juergenzimmermann/pflanze:2025.4.1-bookworm
    docker scout cves --format only-packages juergenzimmermann/pflanze:2025.4.1-bookworm
````
...
- "Feature Filenames", z.B. pflanze.service.ts
